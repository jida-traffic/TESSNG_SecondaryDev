...
void MySimulator::initVehicle(IVehicle *pIVehicle) {
    pIVehicle->setIsPermitForVehicleDraw(true);
    ...
    if (roadName == QString("公路1")) {
        if (tmpId == 4) {
            ...
            pIVehicle->initLane(1, m2p(47), 4);
            pIVehicle->initSpeed(0);
        } else if (tmpId == 5) {
            pIVehicle->setVehiType(1);
            pIVehicle->initLane(1, m2p(10), 9);
        } else if (tmpId == 1) {
            pIVehicle->setVehiType(2);
            pIVehicle->initLane(1, m2p(10), 7);
        } else if (tmpId == 2) {
            pIVehicle->setVehiType(3);
            pIVehicle->initLane(0, m2p(10), 8);
        } else if (tmpId == 3) {
            pIVehicle->setVehiType(2);
            pIVehicle->initLane(1, m2p(30), 6);
        } else if (tmpId == 6) {
            pIVehicle->setVehiType(2);
            pIVehicle->initLane(1, m2p(10), 5);
        } else if (tmpId == 7) {
            pIVehicle->setVehiType(2);
...
bool MySimulator::reCalcToRightFreely(IVehicle *pIVehicle) {
    ...
    QString roadName = pIVehicle->roadName();
    if (roadName == QString("公路1") && simuTime >= 6000) {
        if (pIVehicle->vehicleTypeCode() == 1 || tmpId == 5) {
            if (pIVehicle->vehiDistFront() <= m2p(50)) {
                return true;
            }
        }
    }
