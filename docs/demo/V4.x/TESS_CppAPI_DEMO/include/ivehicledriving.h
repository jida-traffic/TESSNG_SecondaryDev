...
public:
	...
	*/
	...
	// 判断是否第一辆车 onSameRoad为true，只判断相同路段的车辆
	virtual bool isFirstVehi(bool onSameRoad = false);
	/* 车辆距道路起点距离， fromVehiHead:是否从车头计算*/
	virtual qreal distToRoadStart(bool fromVehiHead = false);
	/* 车辆距道路终点距离， fromVehiHead:是否从车头计算*/
	virtual qreal distToRoadEnd(bool fromVehiHead = false);
