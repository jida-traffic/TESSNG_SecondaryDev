...
public class Function {
    ...
    public static Object[] maxband() {
        try {
            // 读取参数配置
            JsonObject params = JsonParser.parseReader(new FileReader("param.json")).getAsJsonObject();
            ...
            // 求解（将 maxIterData 作为参数传入 optimize 方法）
    ...
    public static Map<Integer, Map<String, Object>> calGroups(
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                int originPeriodTime = (int) group.get("period_time");
                int newPeriodTime = (int) Math.round(C);

                @SuppressWarnings("unchecked")
                Map<Integer, Map<String, Integer>> phases =
                        (Map<Integer, Map<String, Integer>>) group.get("phases");

                Map<Integer, Map<String, Integer>> newPhaseDict = new HashMap<>();
                int sumGreenTime = 0;

                for (Map.Entry<Integer, Map<String, Integer>> entry : phases.entrySet()) {
                    int phaseId = entry.getKey();
                    Map<String, Integer> phase = entry.getValue();

                    int startTime;
                    int newGreenInterval;
                    int yellowInterval = phase.get("yellow_interval");
                    int allRedInterval = phase.get("all_red_interval");
                    ...
                    if (corPhases.contains(phaseId)) {
                        ...
                        if (newPhaseDict.isEmpty()) {
                            ...
                            // 找上一个相位
                            Map.Entry<Integer, Map<String, Integer>> lastEntry =
                                    new ArrayList<>(newPhaseDict.entrySet()).get(newPhaseDict.size() - 1);
                            Map<String, Integer> lastPhase = lastEntry.getValue();

                            startTime = lastPhase.get("start_time") +
                                    lastPhase.get("green_interval") +
                                    lastPhase.get("yellow_interval");
                        ...
                        // 按比例分配非协调相位绿灯时间
                        double ratio = (double) (phase.get("green_interval") + phase.get("yellow_interval")) /
                    ...
                    Map<String, Integer> newPhase = new HashMap<>();
                ...
                if (sumGreenTime != newPeriodTime && !newPhaseDict.isEmpty()) {
                    ...
                    lastPhase.put("green_interval", lastPhase.get("green_interval") + adjust);
