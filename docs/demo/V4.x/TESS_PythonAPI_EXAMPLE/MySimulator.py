...
class MySimulator(QObject, PyCustomerSimulator):
    ...
    def calcDynaFlowRatioParameters(self):
        ...
        if batchNum == 20:
            print("DecipointFlowRatioByInterval change")
            # 获取决策点路径
            decisionpoints = iface.decisionPoints
            decisionpoints_new_ratios = []
            if len(decisionpoints) > 0:
                for decisionpoint in decisionpoints:
                    # 一个决策点某个时段各路径车辆分配比
                    dfi = Online.DecipointFlowRatioByInterval()
                    #决策点编号
                    dfi.deciPointID = decisionpoint.id()
                    routings = decisionpoint.routings()
                    ratio = [1 for _ in routings]
                    new_f = []
                    i = 0
                    for routing in routings:
                        ...
                        rfr1 = Online.RoutingFlowRatio(routing.id(), ratio[i])
                        i+=1
                        new_f.append(rfr1)
                    dfi.mlRoutingFlowRatio = new_f
                    decisionpoints_new_ratios.append(dfi)
            return decisionpoints_new_ratios
        return []
    ...
    def afterOneStep(self):
        ...
        if batchNum % 20 == 0:
            print("每20个计算批次做一次小计，将结果通过信号发送出云")
        ...
        # 动态发车，不通过发车点发送，直接在路段和连接段中间某位置创建并发送，每50个计算批次发送一次, 案例中随机选了一个linkId，用户要替换成目标linkId
        if batchNum % 50 == 1:
            print("动态发车，不通过发车点发送，直接在路段和连接段中间某位置创建并发送，每50个计算批次发送一次")
        ...
        # 信号灯组相位颜色
        print("信号灯组相位颜色")
        # lPhoneColor = simuiface.getSignalPhasesColor() # todo 导致闪退
