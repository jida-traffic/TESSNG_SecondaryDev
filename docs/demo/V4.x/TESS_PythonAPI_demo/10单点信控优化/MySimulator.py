...
class MySimulator(QObject, PyCustomerSimulator):
    ...
    def afterOneStep(self):
        ...
        if simuTime / (config.aggregate_time * 1000) > 1 and simuTime % (config.aggregate_time * 1000) == 1000:
            ...
            print()
            print(collector_flow_dict)

            # 每个相位当前绿灯时间
            # group = netiface.signalGroups()[0] # deprecated
            # print(f"group={group}")
            plans = netiface.signalPlans()
            # print(f"plan={plans}")
            plan = netiface.findSignalPlan(201)
            # print(f"plan={plan}")
            cycle_time = plan.periodTime()
            # print(f"phases={plan.Iphases()}")
            phase_interval_dict = {}
            for phase in plan.Iphases():
            ...
            # 计算韦伯斯特信号配时
            new_period_time, new_phase_interval_dict = function.webster(cycle_time, phase_interval_dict, phase_flow_dict)
            print(new_period_time, new_phase_interval_dict)

            # 修改仿真中的信控方案
            # group.setPeriodTime(new_period_time)
            plan.setPeriodTime(new_period_time)
            for phase in plan.Iphases():
