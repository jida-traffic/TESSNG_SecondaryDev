    ...
    def afterOneStep(self):
        ...
        if int(simuTime / (180 * 1000)) == 1 and simuTime % (180 * 1000) == 0:
            ...
            for group_id in cor_groups:
                group = netiface.findSignalPlan(group_id)
                period_time = group.periodTime()
                phase_interval_dict = {}
                for phase in group.Iphases():
                    start_time = 0
            ...
            for group_id, new_group in new_groups.items():
                ...
                for phase_id, new_phase in new_group['phases'].items():
                    print(f"phase_id={phase_id}, new_phase={new_phase}")
                    # phase = netiface.findSignalPhase(phase_id)  # todo 有问题，返回为none
                    phases = group.Iphases()
                    phase = phases[0]
                    for p in phases:
                        # print(f"plan={group.id()}, phase={p.id()}, target phaseid={phase_id}")
                        if p.id()==phase_id:
                            phase = p
                            break
