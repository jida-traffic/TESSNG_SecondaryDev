...
class MyNet(PyCustomerNet):
    ...
    def afterLoadNet(self):
        ...
        # self.ifunctionTest.test(netiface)

        if(count == 0):
            self.ifunctionTest.create_junction(netiface)
            # 节点
            self.ifunctionTest.createJunctionNode(netiface)
            self.ifunctionTest.showJunctionAttr(netiface)

            # 高速公路
            self.ifunctionTest.create_traffic_incident(netiface)
            print("行人信控交叉口模型构建完成")
            # print("打印仿真对象信息")
            self.ifunctionTest.test(netiface)
            self.ifunctionTest.showTessObjAttr(netiface)
